 ...
 *
 * ⚠️ **预扫帧数按 PRESETS[app].preScan 走（微信 12 / 其它 24），并检查返回的 ok 字段。**
 * 下限因 app 而异：微信 12 帧实测稳定；小红书在 12/16 帧时种子会落进状态栏
 * （帧太少时内容区的变化还没体现出来，而时钟和录屏计时胶囊每帧都在变），
 * 返回 y6~68 = 屏高的 3.8%，24 帧起稳定。
 *
...
function stitch(frames, keep, w, viewH) {
  ...
  for (let i = 1; i < grays.length; i++) {
    ...
    if (conf < 0.5) {
      // ⚠️ 本分支必须在 dy<=0 判断【之前】，这是有意的顺序，不是 bug：
      // conf<0.5 说明匹配本身不可信，此时 dy 是垃圾值——它的正负都不携带信息，
      // 一个"低置信的负 dy"不能当回弹处理（走跳过分支会让 anchor 卡死）。
      // 回弹是真实滚动、匹配得上，其 conf 正常在 0.9+，不会落进本分支。
      //
 ...
 *
 * 它优化的是"抽帧次数"（108 → 13）。而 2026-08-28 真机实测宣判了它：
 * 抽帧在真机上只占 10.7s / 247s = 4.3%，**即使抽帧降到 0 也只省 4%**；
 * 而它带来的是一个三次修复失败的已知缺陷——"前段静止后段猛滑"的素材找不到落点
 * （小红书实测 60 次探测只收下 1 帧）。收益 4%、风险高、缺陷未解，账算不过来。
 *
 * 保留代码仅作历史记录与反例参考，任何批次都不要把它接回主路径。
 *

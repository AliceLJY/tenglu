...
第四轮诊断开关会额外对完整 4fps 序列执行 ML Kit OCR，并只导出文本、几何和统计 JSON；
不导出视频或 JPEG，也不把整帧解码成 JavaScript RGBA。算法本身按最大相邻间隔 750 ms
选取 stride=3 子序列，未被选中的临时 JPEG 在 OCR 后立即删除，歧义气泡对应的选中帧保留到
局部采样结束再清理。删除失败会连续尝试最多 3 次；仍有残留时报告

 ...
 *   3. 发言人   —— 左边界峰 = 对方，右边界峰 = 我（微信气泡的对齐是稳定的）
 *   4. 去重     —— 累积位移把每行映射到全局 y，只在邻域内做文本匹配
 *
 * 失败自检见 checkAnchors()：锚点不足或投票分散时报警，不静默降级。
 */
...
const DEDUP_SIM = 0.6;      // 去重的文本相似度阈值
...
const frames = files
  ...
  .map(f => ({ name: f, lines: JSON.parse(fs.readFileSync(path.join(cacheDir, f), "utf8")) }));
...
function checkAnchors(shifts) {
  return shifts
    .map((s, i) => {
      ...
      const reasons = [];
      if (s.total < MIN_ANCHOR) reasons.push(`锚点仅 ${s.total} 个（<${MIN_ANCHOR}）`);
      if (ratio < MIN_VOTE) reasons.push(`位移投票分散（最大簇 ${(ratio * 100).toFixed(0)}% < ${MIN_VOTE * 100}%）`);
...
// ── 4. 去重：全局 y 邻域内做文本匹配，长的覆盖短的
function similarity(a, b) {
  if (a === b) return 1;
  const s = new Set(Array.from(a));
  let hit = 0;
  for (const ch of new Set(Array.from(b))) if (s.has(ch)) hit++;
  const denom = Math.max(s.size, new Set(Array.from(b)).size);
...
function dedupe(lines) {
  ...
  for (const l of sorted) {
    ...
    for (let i = out.length - 1; i >= 0; i--) {
      const p = out[i];
      if (l.gy - p.gy > DEDUP_WINDOW) break;          // 已排序，越界即可停
      if (Math.abs(p.gy - l.gy) > DEDUP_WINDOW) continue;
      if (similarity(norm(p.text), k) < DEDUP_SIM) continue;
...
function renderWechat(lines) {
  ...
  const side = l => {
    if (Math.abs(l.x - left) <= ALIGN_TOL) return "them";
    if (Math.abs(l.x + l.w - right) <= ALIGN_TOL) return "me";
    return "system";
  };
  const msgs = [];
  for (const l of lines) {
    ...
    const dy = cur ? l.gy - cur.lastY : Infinity;
    const dx = cur ? Math.abs(l.x - cur.x) : Infinity;
    if (cur && cur.side === s && dy >= 0 && dy < MERGE_DY && dx < MERGE_DX) {
      cur.text += l.text;
      cur.lastY = l.gy;
    } else {
      msgs.push({ side: s, text: l.text, x: l.x, gy: l.gy, lastY: l.gy });
    }
  }
  return { peaks: { left, right }, text: msgs.map(m => `[${m.side === "me" ? "我" : "对方"}] ${m.text}`).join("\n") };
}

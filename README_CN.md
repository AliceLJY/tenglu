...
- **M2（进行中，第 4 级）**：第 3 级已通过真机验收，输出逐字节一致，总耗时从 247.0 秒降到 164.3 秒。第 4.1 / 4.2 级给相邻帧粗搜和预扫分析使用 quality 0.4 JPEG，选出的关键帧仍以 quality 1 重新抽取并做全分辨率拼接；真机收益与输出一致性等待验证。纯 JS 证明到不了目标后才考虑原生。
